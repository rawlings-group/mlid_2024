...
# y0 = data['ys0']
u0 = np.array([[50], [50]])
y0 = np.array([[53], [46]]) # in degrees C
...
with tclab.TCLab() as lab:
    ...
    def run_experiment(model, u0, y0, ysp, Qs, Rs, label=None):
        ## Get model params
    ...
    run_experiment(base_model, u0, y0, np.zeros((p, Nwarmup)) + y0, Qs, Rs,
    ...
    y = measure(lab)
    ytmp = y0
    print(f'Holding TCLab at Q={u0[:,0]} for {Nwarmup} seconds.')
    ...
    y0 = measure(lab)
    print(f'TCLab settled at T = {y0} C.')
    ysp += y0
    ...
    for (label, model) in models.items():
        ...
        times, u, y = run_experiment(model, u0, y0, ysp, Qs, Rs, label=label)

        experiments[label] = dict(t=np.arange(N), u=u, y=y, ysp=ysp, u0=u0,
                                  y0=y0, model=model, Qs=Qs, Rs=Rs)

## Export data
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
with open(f'tclab_cl_{timestamp}.pickle', 'wb') as handle:

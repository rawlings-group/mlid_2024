...
opt_settings = dict(rho=0., delta=0., mu=0., method='ipopt', verbosity=5,
...
for label in labels:
    ...
    if label == 'ARX':
        ...
        sys = observable_canonical(init_sys, mtype='kf', **model_settings)
        sys.add_eigenvalue_constraint(delta=0.1, beta=100, method='lmi')
        params, stats = sys.fit(udat, ydat)
        A1, B1, C1, _, _, K1, ReL1, *_ = (M.full() for M in params)
        Re1 = ReL1@ReL1.T
        ...
        sys.add_eigenvalue_constraint(delta=0.4, beta=100, method='lmi')
        ...
        sys = observable_canonical(init_sys, mtype='kf', **model_settings)
        sys.add_eigenvalue_constraint(delta=0.1, beta=100)
        params, stats = sys.fit(udat, ydat)
        A1, B1, C1, _, _, K1, ReL1, *_ = (M.full() for M in params)
        Re1 = ReL1@ReL1.T
        ...
        sys.theta0[:n**2] *= 0.59/np.amax(np.abs(np.linalg.eigvals(A0-K0@C0)))
        sys.theta0[n**2+n*m:n**2+n*m+n*p] *= 0.59/np.amax(np.abs(np.linalg.eigvals(A0-K0@C0)))
        sys.add_eigenvalue_constraint(delta=0.4, beta=100)
        ...
        sys = observable_canonical(init_sys, mtype='slti', noise_type='K',
                                   **model_settings, rescale=True, rho=opt_settings['rho'],
                                   epsilon=1e-3)
        params, stats = sys.fit(udat, ydat)
        A1, B1, C1, _, _, K1, ReL1 = (M.full() for M in params)
        Re1 = ReL1@ReL1.T
    t1 = time() - t0
...
with open('data/tclab_mle.pickle', 'wb') as handle:
